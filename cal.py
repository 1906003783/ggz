...
newest_d=dbs.select(sql_string)[0][0]
...
sql_string=f"""create view if not exists type as select char,type \"type\",count(*) \"num\"  from player group by player.type order by char desc;"""
dbs.dbexe(sql_string=sql_string)

dbs.dbexe(f"create view if not exists cnt as select char \"chara\",count(*) \"num\" from player group by player.char order by num DESC;")

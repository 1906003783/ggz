...
class JsonLoader():
    def __init__(self,input,d_halo,col=[],col_c=[],mode="w",encoding="utf-8") -> None:
        self.mode=mode
    ...
    def json2csv(self,out):
        with open(out,mode=self.mode,newline="",encoding=self.encoding) as f2:
            writer=csv.writer(f2)
            writer.writerows(self.loader)
...
if __name__=="__main__":
    ...
    jloader=JsonLoader("data/m.json",d_halo=halo,col=col,col_c=col_c)
    jloader.json2csv("data/rlt.csv")
    jloader=JsonLoader("data/i.json",d_halo=halo,col=col,col_c=col_c)
    jloader.json2csv("data/rlt.csv")
